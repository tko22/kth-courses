...
module.exports = errorWrap(async (req, res) => {
  const {
    query: { id }
  } = req
  ...
  if (!["overallComments", "courseContents", "examination", "literature", "recommendedPrerequisites"].includes(body.type)) {
    res.status(404).send("Make sure your comment type is valid")
    return
  }

  const course = await Courses.findOneAndUpdate({ courseID: id }, {
